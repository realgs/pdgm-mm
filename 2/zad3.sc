...
object zad3 {
  def join [A](list1:List[A], list2:List[A]):List[A] = {
  	@tailrec
  	def joinIter [A](list1:List[A], list2:List[A], acc:List[A]):List[A] =
  		(list1, list2) match {
  			case (Nil, Nil) => acc
  			case (Nil, h2::t2) => joinIter(Nil, t2, acc:::List(h2))
  			case (h1::t1, Nil) => joinIter(t1, Nil, acc:::List(h1))
  			case (h1::t1, h2::t2) => joinIter(t1, t2, acc:::List(h1, h2))
  		}

  	joinIter(list1, list2, List())
